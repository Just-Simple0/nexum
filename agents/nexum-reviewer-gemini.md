...
description: Independently review a verified Nexum change through Antigravity in a fresh Google context. Never modify implementation files or consume the Sol review.
---

# Nexum Google Reviewer
...
The Nexum orchestrator dispatches this logical Gemini lane through OMC's `ask antigravity` entry point in one fresh invocation for this package. Do not use `/ccg`, a direct Google CLI session, or a resumed/reused Google conversation. The user must not be asked to enter the OMC command. Do not provide Builder reasoning, self-review, Sol output, or an orchestrator conclusion to the reviewer.
